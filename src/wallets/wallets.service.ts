...
export class WalletsService {
  constructor(private readonly prisma: PrismaService) {} // ✅ Use DI
 ...
 async verifyTransaction(query: PayStackCallbackDto) {
  ...
  if (!existingTx) {
    ...
    if (status !== 'success') throw new BadRequestException('Payment not successful');
    let user = await this.prisma.user.findUnique({ where: { email: customer.email } });
    if (!user) {
    ...
    let wallet = await this.prisma.wallet.findUnique({ where: { userId: user.id } });
    if (!wallet) {
      wallet = await this.prisma.wallet.create({
        data: { userId: user.id, balance: 0, walletNumber: this.generateWalletNumber() },
      });
    }
  ...
  async handlePaystackWebhook(dto: PaystackWebhookDto, signature: string) {
    const hash = crypto.createHmac('sha512', process.env.PAYSTACK_SECRET!).update(JSON.stringify(dto)).digest('hex');
    if (hash !== signature) return false;

    const reference = dto.data.reference;
    const existingTx = await this.prisma.transaction.findUnique({ where: { reference } });
    if (existingTx) return existingTx;

    const amount = dto.data.amount;
    const userId = dto.data.customer.id;
    return await this.prisma.$transaction(async (prisma) => {
      let wallet = await prisma.wallet.findUnique({ where: { userId } });
      if (!wallet) {
        wallet = await prisma.wallet.create({
          data: { userId, balance: 0, walletNumber: this.generateWalletNumber() },
        });
      }
  ...
  async transfer(userId: string, dto: { walletNumber: string; amount: number }) {
    ...
    const recipientWallet = await this.prisma.wallet.findUnique({ where: { walletNumber: dto.walletNumber } });
